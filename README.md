...
> **🤔 深度思考：为什么 Select 在 100 并发下比 Epoll 还快？**
> *   **轻量级优势**: 在低并发（如 100）场景下，Select 简单的位图轮询机制（线性扫描）开销极小。
> *   **Epoll 开销**: Epoll 在内核维护红黑树和就绪链表，涉及复杂的回调机制。在连接数很少时，这些数据结构的维护成本反而高于 Select 的简单扫描。
> *   **结论**: Select 赢在起跑线（低并发），但 Epoll 赢在扩展性（高并发）。Select 在 1024 连接后会直接失效，而 Epoll 能在 10k+ 连接下保持性能稳定。
...
1.  **临时端口耗尽 (Ephemeral Port Exhaustion)**:
    ...
    *   **结果分析**: 优化后，虽然 QPS 数值略有下降 (46k -> 42k)，但这属于**预期行为**。
        *   未优化前，大量连接因端口耗尽瞬间失败 (Errors: 5864)，失败的连接耗时极短，拉低了平均延迟并虚高了 QPS。
        *   优化后，更多连接成功建立 (Errors: 4763)，服务器需要处理完整的 TCP 握手和数据传输，单次请求耗时增加，反映了服务器处理**真实业务**的极限能力。
